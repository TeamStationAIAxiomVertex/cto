...
import { WithTooltip } from "../../../../../../../../src/components/client/tooltip-hook";

const inputs = {
  // Base cost inputs
  buildIn: { salaryAnnual: 180000, burdenPct: 0.3 },
  ...
  ops: {
    ...
    cfr: {
      ...
      onshore: 0.06,
      offshoreLegacy: 0.08,
      nearshoreLegacy: 0.06,
      nearshoreCoPilot: 0.04,
    },
    teamSize: 12,
    attrition: {
      buildIn: 0.15,
      onshore: 0.15,
      offshoreLegacy: 0.2,
      nearshoreLegacy: 0.16,
      nearshoreCoPilot: 0.12,
    },
    ...
    auditHoursSavedPerYear: {
      buildIn: 80,
      onshore: 80,
      offshoreLegacy: 20,
      nearshoreLegacy: 120,
      nearshoreCoPilot: 250,
    },
...
export default function ComparisonWidget() {
  const [basisHours, setBasisHours] = useState<number>(173);
  const [offshoreOverhead, setOffshoreOverhead] = useState<number>(0.25);
  const [nearshoreLegacyOverhead, setNearshoreLegacyOverhead] =
    useState<number>(0.1);
  const [onshoreOverhead, setOnshoreOverhead] = useState<number>(0.2);
  const [devHourBlended, setDevHourBlended] = useState<number>(
  ...
  const data: Results = useMemo(() => {
    const buildInMonthly =
      (inputs.buildIn.salaryAnnual * (1 + inputs.buildIn.burdenPct)) / 12;

    const onshoreMonthlyLow =
      inputs.onshore.hourlyLow * basisHours * (1 + onshoreOverhead);
    const onshoreMonthlyHigh =
      inputs.onshore.hourlyHigh * basisHours * (1 + onshoreOverhead);

    const offshoreLegacyMonthlyLow =
      inputs.offshoreLegacy.hourlyLow * basisHours * (1 + offshoreOverhead);
    const offshoreLegacyMonthlyHigh =
      inputs.offshoreLegacy.hourlyHigh * basisHours * (1 + offshoreOverhead);

    const nearshoreLegacyMonthlyLow =
      inputs.nearshoreLegacy.hourlyLow *
      basisHours *
      (1 + nearshoreLegacyOverhead);
    const nearshoreLegacyMonthlyHigh =
      inputs.nearshoreLegacy.hourlyHigh *
      basisHours *
      (1 + nearshoreLegacyOverhead);

    const nearshoreCoPilotMonthlyLow =
      inputs.nearshoreCoPilot.hourlyMin * basisHours;
    const nearshoreCoPilotMonthlyHigh =
      inputs.nearshoreCoPilot.hourlyCap * basisHours;

    const prLatency = {
      buildIn:
        inputs.ops.prsPerMonth *
        (inputs.ops.reviewHours.buildIn - 1.0) *
        devHourBlended,
      onshore:
        inputs.ops.prsPerMonth *
        (inputs.ops.reviewHours.onshore - 1.0) *
        devHourBlended,
      offshoreLegacy:
        inputs.ops.prsPerMonth *
    ...
    const vacancy = {
      buildIn:
        ...
        (inputs.ops.ttoDays.buildIn - inputs.ops.ttoDays.nearshoreCoPilot),
      onshore:
        inputs.ops.dailyValueUSD *
        (inputs.ops.ttoDays.onshore - inputs.ops.ttoDays.nearshoreCoPilot),
      offshoreLegacy:
        inputs.ops.dailyValueUSD *
    ...
    const mgmt = {
      ...
      offshoreLegacy:
        inputs.ops.mgmtHoursMonth.offshoreLegacy.EM *
          inputs.ops.mgmtRatesUSD.EM +
        inputs.ops.mgmtHoursMonth.offshoreLegacy.PM *
          inputs.ops.mgmtRatesUSD.PM,
      nearshoreLegacy:
        inputs.ops.mgmtHoursMonth.nearshoreLegacy.EM *
          inputs.ops.mgmtRatesUSD.EM +
        inputs.ops.mgmtHoursMonth.nearshoreLegacy.PM *
          inputs.ops.mgmtRatesUSD.PM,
      nearshoreCoPilot: 0,
    };
    ...
    const compliance = {
      buildIn:
        (inputs.ops.auditHoursSavedPerYear.buildIn *
          inputs.ops.complianceHourRateUSD) /
        12,
      onshore:
        (inputs.ops.auditHoursSavedPerYear.onshore *
          inputs.ops.complianceHourRateUSD) /
        12,
      offshoreLegacy:
        (inputs.ops.auditHoursSavedPerYear.offshoreLegacy *
          inputs.ops.complianceHourRateUSD) /
        12,
      nearshoreLegacy:
        (inputs.ops.auditHoursSavedPerYear.nearshoreLegacy *
          inputs.ops.complianceHourRateUSD) /
        12,
      nearshoreCoPilot:
        (inputs.ops.auditHoursSavedPerYear.nearshoreCoPilot *
          inputs.ops.complianceHourRateUSD) /
        12,
    };
    ...
    const total = {
      buildIn:
        buildInMonthly + prLatency.buildIn + mgmt.buildIn - compliance.buildIn,
      onshore:
        (onshoreMonthlyLow + onshoreMonthlyHigh) / 2 +
        prLatency.onshore +
        mgmt.onshore -
        compliance.onshore,
      offshoreLegacy:
        (offshoreLegacyMonthlyLow + offshoreLegacyMonthlyHigh) / 2 +
        prLatency.offshoreLegacy +
        mgmt.offshoreLegacy -
        compliance.offshoreLegacy,
      nearshoreLegacy:
        (nearshoreLegacyMonthlyLow + nearshoreLegacyMonthlyHigh) / 2 +
        prLatency.nearshoreLegacy +
        mgmt.nearshoreLegacy -
        compliance.nearshoreLegacy,
      nearshoreCoPilot:
        (nearshoreCoPilotMonthlyLow + nearshoreCoPilotMonthlyHigh) / 2 -
        compliance.nearshoreCoPilot,
    };
    ...
    return {
      ...
      effectiveHourly: [
        ...
        fRange(
          offshoreLegacyMonthlyLow / basisHours,
          offshoreLegacyMonthlyHigh / basisHours
        ),
        fRange(
          nearshoreLegacyMonthlyLow / basisHours,
          nearshoreLegacyMonthlyHigh / basisHours
        ),
        fRange(
          nearshoreCoPilotMonthlyLow / basisHours,
          nearshoreCoPilotMonthlyHigh / basisHours
        ),
      ],
      ...
      <div className="mt-6 flex flex-wrap justify-center items-center gap-x-6 gap-y-4 rounded-lg bg-background p-4 border text-sm">
        <div className="flex items-center gap-2">
          <WithTooltip
            ...
            content={
              <span>
                The number of billable hours per month. 173 is standard for
                full-time.
              </span>
            }
          >
        ...
        <div className="flex items-center gap-2">
          <WithTooltip
            label="Blended Dev Rate"
            content={
              <span>
                Your blended engineering cost per hour, used for calculating
                latency and rework costs.
              </span>
            }
          >
            <label
              htmlFor={`${baseId}-dev-hour`}
              className="font-medium text-muted-foreground border-b border-dashed"
            >
        ...
        <div className="flex items-center gap-2">
          <WithTooltip
            label="Onshore Overhead"
            content={
              <span>
                Estimated management and administrative overhead for onshore
                vendors.
              </span>
            }
          >
          ...
          <input
            ...
            onChange={(e) =>
              setOnshoreOverhead(clamp(e.target.valueAsNumber, 0, 100) / 100)
            }
        ...
        <div className="flex items-center gap-2">
          <WithTooltip
            label="Offshore Overhead"
            content={
              <span>
                Estimated management, communication, and rework overhead for
                offshore vendors.
              </span>
            }
          >
          ...
          <input
            ...
            onChange={(e) =>
              setOffshoreOverhead(clamp(e.target.valueAsNumber, 0, 100) / 100)
            }
            className="w-16 bg-background border border-border rounded-md px-2 py-1 focus:ring-2 focus:ring-primary"
          />
          <span className="text-muted-foreground">%</span>
        ...
        <div className="flex items-center gap-2">
          <WithTooltip
            label="Legacy Nearshore Overhead"
            content={
              <span>
                Estimated management and administrative overhead for legacy
                nearshore vendors.
              </span>
            }
          >
          ...
          <input
            ...
            onChange={(e) =>
              setNearshoreLegacyOverhead(
                clamp(e.target.valueAsNumber, 0, 100) / 100
              )
            }
            className="w-16 bg-background border border-border rounded-md px-2 py-1 focus:ring-2 focus:ring-primary"
          />
