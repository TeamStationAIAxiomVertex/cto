...
import { roleCategories } from "../../../src/lib/roles";
...
const companyLinks: LinkItem[] = [
  ...
  { href: "/pricing", label: "Pricing & Engagement Models" },
  {
    ...
    label: "Nearshore IT Platformed Book",
  },
  {
    href: "https://scholar.google.com/citations?user=aNol-ycAAAAJ&hl=en",
    label: "Google Scholar Research",
  },
  {
    href: "https://www.linkedin.com/company/teamstation",
    label: "TeamStation LinkedIn",
  },
];
...
const researchLinks: LinkItem[] = [
  {
    ...
    label: "AxiomCortex™ R&D Report",
  },
  {
    href: "/research/heuristically-trained-ai",
    label: "Heuristically Trained AI Whitepaper",
  },
  {
    href: "/research/framework-for-measuring-capacity",
    label: "Framework for Measuring Engineering Capacity",
  },
  {
    href: "/research/performance-metrics-in-ai-age",
    label: "Performance Metrics in the AI Age",
  },
  {
    ...
    label: (
      <>
        TeamStation Podcast{" "}
        <SpotifyIcon className="h-4 w-4 inline-block ml-1 align-text-bottom" />
      </>
    ),
  },
];
...
const popularTechLinks: LinkItem[] = [
  ...
  const tech = techCategories
    .flatMap((c) => c.tech)
    .find((t) => t.slug === slug);
  return {
    href: `/hire/by-technology/${slug}`,
    label: `Hire ${tech?.name || slug} developers`,
  };
});
...
function LinkColumn({ title, links }: { title: string; links: LinkItem[] }) {
  return (
    <nav aria-label={title}>
      <h4 className="font-semibold text-foreground tracking-wider uppercase text-sm">
        {title}
      </h4>
      ...
      <div className="container mx-auto max-w-7xl px-6">
        <div className="grid grid-cols-2 gap-y-10 gap-x-8 md:grid-cols-12">
          <div className="col-span-2 md:col-span-3">
            ...
            <p className="mt-2 text-sm">
              The integrated platform for building and scaling elite nearshore
              engineering teams.
            </p>
            ...
            <div className="mt-4">
              <Link
                ...
                className="text-sm font-medium text-primary hover:text-primary/80 a11y-tap-target"
              >
                Sign In to Platform
              </Link>
            </div>
          ...
          <div className="col-span-1 md:col-span-2">
            <LinkColumn
              title="Hire by Role"
              links={hireByRoleLinks.slice(0, 11)}
            />
          </div>
        ...
        <div className="mt-16 border-t border-border pt-8 text-center text-sm">
          ...
          <p className="mt-2">
            ...
            <a
              href="https://teamstation.dev"
              target="_blank"
              rel="noopener noreferrer"
              className="font-semibold text-primary hover:underline a11y-tap-target"
            >
              teamstation.dev
            </a>
          </p>
          <div className="flex flex-wrap justify-center gap-x-4 gap-y-2 mt-4">
            {utilityLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="text-muted-foreground hover:text-foreground a11y-tap-target"
              >
                {link.label}
              </Link>
            ))}
