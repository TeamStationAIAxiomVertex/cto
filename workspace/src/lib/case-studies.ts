...
export type CaseStudy = { slug: string; clientName?: string; industry?: string; summary?: string; lastModified?: string; ogImage?: { src?: { url?: string }; aiHint?: string }; contentHtml?: string; };
...
export async function getCaseStudyBySlug(slug: string | undefined){ if (!slug) return null; const dir=findDir(); if(!dir) return null; const file=[".md",".mdx"].map(ext=>path.join(dir,`${slug}${ext}`)).find(f=>fs.existsSync(f)); if(!file) return null; const raw=fs.readFileSync(file,"utf8"); const {data,content}=matter(raw); const html=await markdownToHtml(content); const stat=fs.statSync(file); return { slug, clientName:data.clientName||data.title, industry:data.industry, summary:data.summary||data.description, ogImage:data.ogImage, lastModified:stat.mtime.toISOString(), contentHtml:html }; }
