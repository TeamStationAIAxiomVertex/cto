...
export async function generateMetadata({
  ...
  const titleSafe = (study.title ?? "Case Study").replace(
    / \| TeamStation AI( Case Study)?/g,
    " Case Study"
  );
  const summarySafe = (study.summary ?? "").trim();
  const fallbackSummary = `Case study: how TeamStation AI helped ${
    study.clientName ?? "a client"
  }${study.industry ? ` with ${study.industry} challenges` : ""}.`.trim();
  const description =
    summarySafe.length > 160 ? fallbackSummary : summarySafe || fallbackSummary;
    ...
    <>
      ...
      <main className="container max-w-7xl py-12">
        <div className="text-sm text-muted-foreground mb-8">
          ...
          / <span>{study.clientName ?? titleSafe}</span>
        </div>
          ...
          <aside className="lg:col-span-4 lg:sticky top-24 self-start space-y-8">
            ...
            <div className="rounded-xl border bg-card text-card-foreground p-6 shadow-lg">
              ...
              <p className="mt-4 text-sm text-foreground">
                {summarySafe ||
                  "A comprehensive solution was implemented to address the client's challenges."}
              </p>
            </div>
            ...
            <div className="text-center rounded-lg bg-primary/10 p-8 shadow-lg">
              ...
              <p className="mt-2 text-sm text-muted-foreground">
                Let TeamStation AI provide the talent, governance, and
