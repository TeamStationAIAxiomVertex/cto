...
import { getCountryData, getCountrySlugs, CountryFactPack, generateFaqSchema, generateVerdictRows } from '@/lib/programmatic-data'; 
...
export default async function HireByCountryPage({ params }: { params: { slug: string } }) {
  const data = await getCountryData(params.slug);
  ...
  return (
    <main className="container mx-auto px-4 py-8 max-w-7xl">
      ...
      <script 
        type="application/ld+json" 
        dangerouslySetInnerHTML={{ __html: JSON.stringify({ /* BreadcrumbList Schema here */ }) }} 
      />
      ...
      <section className="prose lg:prose-xl max-w-none">
        <ProgrammaticContent slug={params.slug} contentType="hire-country" /> 
      </section>

      <section className="mt-12">
        <h2 className="text-3xl font-bold mb-6">Why TeamStation AI Wins in {country}</h2>
        ...
        <div className="grid md:grid-cols-2 gap-8 mt-12">
            <div className="bg-yellow-50 p-6 rounded-lg border-l-4 border-yellow-500">
                <h3 className="text-xl font-semibold mb-2 text-yellow-800">Legal and EOR Guidance</h3>
                <p className="text-gray-700">{legal_note}</p>
            </div>
            <div className="bg-blue-50 p-6 rounded-lg border-l-4 border-blue-500">
                <h3 className="text-xl font-semibold mb-2 text-blue-800">Compensation Insights</h3>
