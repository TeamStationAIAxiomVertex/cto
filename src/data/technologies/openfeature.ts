...
const openfeature: TechEntry = {
  ...
  pains: [
    ...
    },
    {
      ...
      problem:
        "When feature flag logic is spread throughout the codebase using different client libraries and patterns, it becomes difficult to manage and reason about. This leads to an inconsistent user experience and makes it hard to get a clear picture of which flags are enabled for which users.",
      solution:
        "Our engineers are masters of <strong>declarative feature management</strong>. They are vetted on their ability to use OpenFeature to provide a single, consistent API for feature flagging across your entire application, regardless of the programming language or framework being used.",
    ...
    },
    {
      ...
      problem:
        "To make powerful, targeted decisions, your feature flags need context about the user, the request, and the environment. Passing this context to a proprietary SDK can be complex and requires custom code for each provider.",
      solution:
        "We look for engineers proficient with the <strong>OpenFeature Evaluation Context</strong>. They are vetted on their ability to use the Evaluation Context to pass rich, structured data to the feature flagging provider, enabling powerful, context-aware targeting rules without vendor-specific code.",
