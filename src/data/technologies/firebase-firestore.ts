...
const firebaseFirestore: TechEntry = {
  ...
  meta_description:
    "Build a scalable, real-time, and serverless application. Hire elite remote Firestore experts vetted for their deep understanding of NoSQL, from the data model to the security rule. Book a call.",
  intro:
    "Firestore is a flexible, scalable database for mobile, web, and server development from Firebase and Google Cloud Platform. You need an engineer who understands that building a modern, real-time application is not just about writing code, but about a deep understanding of the entire NoSQL ecosystem, from the data model and the query to the security rule and the index. Our vetting process identifies developers who are experts in the modern data stack and can architect a data platform that is as secure, efficient, and automated as it is scalable. By hiring a Firestore expert, you get a developer who can build a modern, automated, and observable system that is a joy to use, and a joy to maintain.",
  pains: [
    {
      icon: AlertTriangle,
      pain: "Is your data infrastructure a mess of slow, inefficient queries?",
      problem:
        "In many organizations, the data infrastructure is a mess of slow, inefficient queries. This can lead to a user experience that is filled with stale data and a codebase that is a mess of environment-specific hacks and workarounds.",
      solution:
        "We vet for experts in **Firestore and the modern data stack**. Our engineers are proficient in using tools like dbt, Airflow, and Spark to build a fully automated, reliable, and efficient data infrastructure. This results in a codebase that is easy to understand and a user experience that is a joy to use.",
  ...
  evaluation: [
    "The modern data stack (e.g., Firestore, dbt, other modern libraries)",
    "Firestore and the modern web",
    "The data model and the query",
    "The security rule and the index",
    "Security and best practices",
  ],
  technical_analysis:
    "The Firestore evaluation focuses on a candidate's ability to build a scalable, real-time, and serverless application. We assess their understanding of the entire Firestore ecosystem, requiring them to design a complex application with a clean, automated, and observable architecture. A critical part of the evaluation is their mastery of **Firestore and the modern data stack**; candidates must demonstrate how to use tools like Firestore, dbt, and other modern libraries to build a modern, scalable, and resilient data infrastructure, and how to use the data model and the query to build a read model that is optimized for queries. We also test their ability to build **portable and environment-agnostic** applications that can run on any environment. Finally, we assess their knowledge of the data ecosystem, and their ability to leverage it to build a modern, high-performance backend.",
  interlink_slugs: ["firebase", "google-cloud", "react", "vue"],
};
