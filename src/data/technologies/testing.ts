...
const testing: TechEntry = {
  ...
  seo_title:
    "Hire React Testing Library Experts | Frontend Testing Consultants",
  meta_description:
  ...
  pains: [
    {
      ...
      problem:
        "Testing implementation details (e.g., component state, method names) leads to brittle tests that break every time you refactor your code. This makes testing a chore that developers avoid, leading to a decline in code quality and an increase in production bugs.",
      solution:
        "We vet for experts in <strong>user-centric testing</strong>. Our engineers are proficient in using React Testing Library to write tests that resemble how users interact with your application. This results in a codebase that is easy to refactor and a test suite that is resilient to implementation changes.",
